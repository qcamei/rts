...
postFun = function(eqs,city,town){  //後台地址
  var cityselect = $(".m_city").eq(eqs);
  var townselect = $(".m_town").eq(eqs);  
  var citys = cityFun(city);
  cityselect.append(citys).trigger("chosen:updated");      
  //cityselect.append(citys).selectpicker("refresh");   
  var towns = townFun(eqs,town);         
  townselect.append(towns).trigger("chosen:updated");     
  //townselect.append(towns).selectpicker("refresh");   
  zipFun(eqs);   

  cityselect.change(function(){                
     var towns = townFun(eqs,'');            
     townselect.find("option").remove().trigger("chosen:updated");  
     townselect.append(towns).trigger("chosen:updated");    
     zipFun(eqs);         
  });
...
postFun2 = function(eqs,city,town){  //前台地址
  var cityselect = $(".m_city").eq(eqs);
  var townselect = $(".m_town").eq(eqs); 
  var citys = cityFun(city);  
  cityselect.append(citys).trigger("chosen:updated");   
...
cityFun = function(fcity){
  var citys = "<option value=''>選擇縣市</option>"
  for (var i=1;i<County.length;i++){
   citys += "<option value='" + County[i] + "'";
   if (County[i] == fcity){citys += " selected='selected' "}
...
townFun = function(eqs,ftown){
  ...
  var towns = "<option value=''>鄉鎮區</option>"  
