...
$.extend($.validator, {
    ...
    submitHandler:function(form) {    
      ...
      actionStr = $(form).attr('action');    
        $('#submit').attr('disabled',true); 
	      ...
	      $.post($(form).attr('action'),$(form).serialize(),function(data){      	
	      	if (data.indexOf(".asp") == -1){      		      		
	      		if (data != ""){	      		 
	      			if (actionStr == "member-password.asp"){
		      	  ...
		      	  if (data.indexOf("@") != -1){
		      	  	...
		      	  	switch(dataDim[1]){
		      	  		case "salert": salert(dataDim[0]);break;
		      	  		case "ealert": ealert(dataDim[0]);break;
		      	  		case "walert": walert(dataDim[0]);break;
		      	  		case "nalert": nalert(dataDim[0]);break;
		      	  	}
		      	  }else{
		      	    $(".container").find(".formMain").hide();
		      	    $(".container").find(".formContent").html("<h2>"+data+"</h2>");
		      	  }
	      		...
	      		//if ($("#captcha").length != 0){
	    			//	var sec = new Date();
	    			//	$("#captcha").attr("src","captcha/captcha.asp?sec="+sec); 
	    			//}
	      	}else{       	
	      ...
	      //這個只能放在post的下面，不能放裡面
			 ...
			 //      close: function() {		      		
				//       if (errorMsg == false){
				//         $(form)[0].reset();		        
				//         $("#small-dialog").find(".errMsg").hide();
				//         $("#small-dialog").find(".success").show();
				//       }
			 //    	}
			 //    }
				// });
	    ...
	    }else if (element.attr('name') == "Rndno"){
	    	$("#captcha").after(error);
	    }else{
	...
	prototype: {
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				// create label
				if (this.idOrName(element) == "Rndno"){					
					label = $("<" + this.settings.errorElement + "/>")
					.attr({"for":  this.idOrName(element), generated: true})
					.addClass(this.settings.errorClass +" error2")
					.html(message || "");
				}else{
					label = $("<" + this.settings.errorElement + "/>")
					.attr({"for":  this.idOrName(element), generated: true})
